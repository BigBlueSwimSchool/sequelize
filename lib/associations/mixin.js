...
var Utils = require('./../utils')
  ...
  , HasOneOverTime = require('./has-one-over-time')
...
Mixin.hasOneOverTime = function(targetModel, options) { // testhint options:none
  if (!(targetModel instanceof this.sequelize.Model)) {
    throw new Error(this.name + '.hasOneOverTime called with something that\'s not an instance of Sequelize.Model');
  }
  ...
  // the id is in the foreign table or in a connecting table
  var association = new HasOneOverTime(sourceModel, targetModel, options);
  sourceModel.associations[association.associationAccessor] = association.injectAttributes();
